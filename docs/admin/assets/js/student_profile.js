...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchStudentData(studentId) {
        const [coursesRes, gradesRes, enrollmentRes, documentsRes] = await Promise.all([
        ...
        return {
            courses: {
                ...
                currentCourses: coursesRes
                    ...
                    .map(c => `${c.course_name} (${c.credits} credits)`),
            ...
            grades: {
                current: gradesRes
                    ...
                    .map(g => ({ course: g.course_name, grade: g.grade })),
                previous: gradesRes
            ...
            enrollment: {
                ...
                status: enrollmentRes.enrollment_status || "",
                ...
                ${
                    g.current?.length
                        ...
                        <table>
                            <thead><tr><th>Course</th><th>Grade</th></tr></thead>
                            <tbody>${g.current
                                .map(r => `<tr><td>${r.course}</td><td>${r.grade}</td></tr>`)
                                .join("")}</tbody>
                        </table>`
    ...
    tabContainer.addEventListener("click", async ev => {
        ...
        const studentId = urlParams.get("id") || "S1001";
    ...
    // Auto-load first tab
    const initial = tabContainer.querySelector(".tab-link.active")
