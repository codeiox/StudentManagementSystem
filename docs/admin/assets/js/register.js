document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`/api/admin/students/${studentId}`)
        ...
        .then((student) => {
            ...
            if (gender === 'female') {
                photo.src = 'https://api.dicebear.com/9.x/adventurer/svg?seed=Jude';
            } else if (gender === 'male') {
                photo.src = 'https://api.dicebear.com/9.x/adventurer/svg?seed=Luis';
            } else {
                photo.src = 'https://api.dicebear.com/9.x/adventurer/svg?seed=other';
            }
        })
        .catch((err) => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const degreeType = degreeTypeSelect.value; // major or minor
        const major_id = majorSelect.value;

        if (!studentId || !course_id || !term || !degreeType || !major_id) {
            messageDiv.textContent = 'Missing required fields: studentId, course_id, term, degreeType, or major_id';
            messageDiv.style.color = 'red';
            return;
        }
        ...
        try {
            const response = await fetch(`/api/admin/students/${studentId}/enrollments`, {
                ...
                body: JSON.stringify({
                    ...
                    term,
                    major_id: degreeType === 'major' ? parseInt(major_id) : undefined,
                    minor_id: degreeType === 'minor' ? parseInt(major_id) : undefined,
                }),
            });
            ...
            if (response.ok) {
                ...
                // Optionally, refresh the right panel info after updating major/minor
